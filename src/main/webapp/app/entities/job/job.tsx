  ...
  return (
    <div>
      <h2 id="job-heading" data-cy="JobHeading">
        <Translate contentKey="nilanApp.job.home.title">Jobs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="nilanApp.job.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/job/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="nilanApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </div>
      </h2>
        ...
        {jobList && jobList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="nilanApp.job.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('jobTitle')}>
                  <Translate contentKey="nilanApp.job.jobTitle">Job Title</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('jobTitle')} />
                </th>
                <th className="hand" onClick={sort('minSalary')}>
                  <Translate contentKey="nilanApp.job.minSalary">Min Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('minSalary')} />
                </th>
                <th className="hand" onClick={sort('maxSalary')}>
                  <Translate contentKey="nilanApp.job.maxSalary">Max Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('maxSalary')} />
                </th>
                <th>
                  <Translate contentKey="nilanApp.job.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="nilanApp.job.home.notFound">No Jobs found</Translate>
            </div>
          )
        )}
      </div>
